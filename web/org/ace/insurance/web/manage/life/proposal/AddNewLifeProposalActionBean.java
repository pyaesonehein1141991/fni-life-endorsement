...
public class AddNewLifeProposalActionBean extends BaseBean implements Serializable {
	...
	public boolean isCreateNewInsuredPersonInfo() {
		// beneficaryCloneDTO = new BeneficiariesInfoDTO();
		// beneficaryCloneDTO.setIdType(IdType.NRCNO);
	...
	private boolean validInsuredPerson(InsuredPersonInfoDTO insuredPersonInfoDTO) {
		...
		int personAge = insuredPersonInfoDTO.getDateOfBirth() != null ? insuredPersonInfoDTO.getAgeForNextYear() : 0;
	...
	private void saveInsuredPersonInfo() {
		...
		if (validInsuredPerson(insuredPersonInfoDTO)) {
			...
			if (!isEndorse) {
				int personAge = insuredPersonInfoDTO.getDateOfBirth() != null ? insuredPersonInfoDTO.getAgeForNextYear() : 0;
				setKeyFactorValue(insuredPersonInfoDTO.getSumInsuredInfo(), personAge, lifeProposal.getPeriodMonth(), insuredPersonInfoDTO.getIsRiskyOccupation(),
						insuredPersonInfoDTO.getPounds(), insuredPersonInfoDTO.getRiskyOccupation() == null ? 0 : insuredPersonInfoDTO.getRiskyOccupation().getExtraRate());
				// For Endorsement
			} else {
