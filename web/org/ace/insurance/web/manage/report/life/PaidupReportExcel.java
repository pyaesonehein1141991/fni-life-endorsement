...
public class PaidupReportExcel {

	private XSSFWorkbook wb;
	// private FormulaEvaluator evaluator ;

	public PaidupReportExcel() {
		load();
	}

	private void load() {
		try {
			InputStream inp = this.getClass().getResourceAsStream("/report-template/life/PaidupReport.xlsx");
			wb = new XSSFWorkbook(inp);
		} catch (IOException e) {
			throw new SystemException(ErrorCode.SYSTEM_ERROR, "Failed to load PaidupReport.xlsx tempalte", e);
		}
	}

	public Map<String, List<PaidupReportDTO>> separateByPaymentChannel(List<PaidupReportDTO> paiduplist) {
		Map<String, List<PaidupReportDTO>> map = new LinkedHashMap<String, List<PaidupReportDTO>>();

		return map;

	}

	public void generate(OutputStream op, List<PaidupReportDTO> paiduplist) {
		try {
			Sheet sheet1 = wb.getSheet("PaidupReport");
			...
			for (PaidupReportDTO mktforlifeDto : paiduplist) {
				...
				cell.setCellValue(mktforlifeDto.getRealPaidupAmount());
				cell.setCellStyle(textCellStyle);
			...
			i = i + 1;
			sheet1.addMergedRegion(new CellRangeAddress(i, i, 0, 14));
			row = sheet1.createRow(i);
			cell = row.createCell(0);
