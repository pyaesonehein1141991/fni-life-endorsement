...
public class APEReportExcelforHealth {

	private XSSFWorkbook wb;
	// private FormulaEvaluator evaluator ;

	public APEReportExcelforHealth() {
		load();
	}
	...
	public Map<String, List<APEReportforHealthDTO>> separateByPaymentChannel(List<APEReportforHealthDTO> apelistforhealth) {
		Map<String, List<APEReportforHealthDTO>> map = new LinkedHashMap<String, List<APEReportforHealthDTO>>();
		if (apelistforhealth != null) {
			for (APEReportforHealthDTO report : apelistforhealth) {
				if (map.containsKey(report.getProductName())) {
					map.get(report.getProductName()).add(report);
				} else {
					List<APEReportforHealthDTO> list = new ArrayList<APEReportforHealthDTO>();
					list.add(report);
	...
	public void generate(OutputStream op, List<APEReportforHealthDTO> apelistforhealth) {
		try {
			...
			Date date = apelistforhealth.get(0).getPaymentDate();
			if (date != null) {
			...
			for (APEReportforHealthDTO apeDto : apelistforhealth) {
