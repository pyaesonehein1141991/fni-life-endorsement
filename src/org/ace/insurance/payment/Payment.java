...
@NamedQueries(value = { @NamedQuery(name = "Payment.findAll", query = "SELECT m FROM Payment m "),
		...
		@NamedQuery(name = "Payment.findByReferenceNo", query = "SELECT m FROM Payment m WHERE ((m.isReverse=false and m.complete=true) or (m.isReverse=false and m.complete=false)) and (m.referenceNo = :referenceNo) ORDER BY m.paymentDate"),
		...
		@NamedQuery(name = "Payment.reversePaymentByReceiptNo", query = "UPDATE Payment p SET p.isReverse = true WHERE p.invoiceNo = :invoiceNo"),
		@NamedQuery(name = "Payment.findPaymentTermByReferenceNo", query = "SELECT m FROM Payment m WHERE m.isReverse = false and m.complete = true and m.referenceNo = :referenceNo"),
		@NamedQuery(name = "Payment.findByPolicyNoWithNotNullReceiptNo", query = "SELECT m FROM Payment m WHERE m.isReverse = false and m.complete = true and m.receiptNo!='-'and m.referenceType!=:referenceType and m.policyNo = :policyNo"),
		@NamedQuery(name = "Payment.findPaymentListByReferenceNo", query = "SELECT m FROM Payment m WHERE m.isReverse = false and m.complete = true and m.referenceNo = :referenceNo")})
...
public class Payment implements Serializable, IDataModel {
	...
	@Transient
	private double newEndrosePremium;
	...
	public void setPreIncomeAmount(double preIncomeAmount) {
	...
	public double getNewEndrosePremium() {
		return newEndrosePremium;
	}

	public void setNewEndrosePremium(double newEndrosePremium) {
		this.newEndrosePremium = newEndrosePremium;
	}
