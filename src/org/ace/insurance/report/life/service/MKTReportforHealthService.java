...
@Service(value = "MKTReportforHealthService")
public class MKTReportforHealthService implements IMKTReportforHealthService {
	
	@Resource(name = "MKTReportforHealthDAO")
	private IMKTReportforHealthDAO mktReportforHealthDAO;

	@Transactional(propagation = Propagation.REQUIRED)
	public  List<MKTforHealthReportDTO> findmktforhealth(MKTforHealthReportCriteria mktforhealthReportCriteria) {
		List<MKTforHealthReportDTO> result = null;
		try {
			result = mktReportforHealthDAO.find(mktforhealthReportCriteria);
	...
	public void generatemktforHealthReport(List<MKTforHealthReportDTO> reportList, String dirPath, String fileName, String branch) {
		try {
			...
			for (MKTforHealthReportDTO report : reportList) {
	...
	private double getTotalAmount(List<MKTforHealthReportDTO> reportList) {
		double totalAmount = 0.0;
		for (MKTforHealthReportDTO mktforlifeReportDTO : reportList) {
