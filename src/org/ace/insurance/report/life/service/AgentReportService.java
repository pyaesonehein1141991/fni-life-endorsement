...
@Service(value = "AgentReportService")
public class AgentReportService implements IAgentReportService {
	
	@Resource(name = "AgentReportDAO")
	private IagentReportDAO agentReportDAO;

	@Transactional(propagation = Propagation.REQUIRED)
	public  List<AgentReportDTO> findagent(AgentReportCriteria agentReportCriteria) {
		List<AgentReportDTO> result = null;
		try {
			result = agentReportDAO.find(agentReportCriteria);
	...
	public void generateagentReport(List<AgentReportDTO> reportList, String dirPath, String fileName, String branch) {
		try {
			...
			for (AgentReportDTO report : reportList) {
			...
			InputStream policyIS = Thread.currentThread().getContextClassLoader().getResourceAsStream("report-template/life/agentReport.jrxml");
